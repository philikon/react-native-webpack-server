...
class Server {
  ...
  start() {
    ...
    return this._writeEntryFiles().then(() => {
      log('Server: entry files written()');
      ...
      const listenPromise = new Promise(resolve => {
        this.httpServer = this.server.listen(this.port, () => {
          log('Server: rnws server listening');
  ...
  handleBundleRequest(req, res, next) {
    log('Server.handleBundleRequest()');
  ...
  _startPackageServer() {
    log('Server._startPackageServer()');
    /**
    ...
    return new Promise((resolve, reject) => {
      ...
      function handleError(err) {
        log('Server: package server handleError()');
        reject(err);
      }
      ...
      waitForSocket({ port: this.packagerPort, tries: 120 }, err => {
        log('Server: react-native packager listening');
  ...
  _startWebpackDevServer() {
    log('Server._startWebpackDevServer()');
    ...
    return getReactNativeExternals({
      ...
      log('Server: resolved react-native externals');
      ...
      const compilerPromise = new Promise(resolve => {
        compiler.plugin('done', () => {
          log('Server: compilerPromise resolved');
      ...
      const serverPromise = new Promise(resolve => {
        this.webpackServer.listen(this.webpackPort, this.hostname, () => {
          log('Server: webpack server listening');
