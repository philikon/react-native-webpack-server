...
class Server {
   ...
   * @param {String}  packagerCacheDir Directory in which to cache the React Native bundle and source maps
   *
   * @constructor
  ...
  constructor(options) {
    ...
    this.packagerCacheDir = options.packagerCacheDir;
  ...
  fetchPackagerResource(type, query) {
    if (!this.packagerCacheDir) {
      return this.makePackagerRequest(type, query);
    }
    ...
    const filename = [
      ...
      query.platform,
      query.dev === 'true' ? 'dev' : '',
      query.minify === 'true' ? 'min' : '',
      type,
    ].filter(x => !!x).join('.');
    const absoluteFilename = path.join(this.packagerCacheDir, filename);
    if (fs.existsSync(absoluteFilename)) {
      console.log('Loading from cached file ' + absoluteFilename);
      return fs.readFileAsync(absoluteFilename, {encoding: 'utf-8'});
    }

    console.log(absoluteFilename + ' not found, loading from packager server.');
    return this._ensurePackageServer()
      .then(() => this.makePackagerRequest(type, query))
      .then(res => {
        console.log('Storing packager result to cached file at ' + absoluteFilename);
        return fs.writeFileAsync(absoluteFilename, res, {encoding: 'utf-8'}).then(() => res);
      });
  }

  makePackagerRequest(type, query) {
    const packagerUrl = url.resolve(this.packagerBaseURL, `index.${query.platform}.${type}`);
    const packagerUrlInfo = url.parse(packagerUrl);
    // Forward URL params to RN packager
    packagerUrlInfo.query = query;
    return fetch(url.format(packagerUrlInfo));
  }
  ...
  handleBundleRequest(req, res, next) {
    ...
    Promise.props({
      reactCode: this.fetchPackagerResource('bundle', parsedUrl.query),
  ...
  handleMapRequest(req, res, next) {
    ...
    Promise.props({
      reactCode: this.fetchPackagerResource('bundle', parsedUrl.query),
      reactMap: this.fetchPackagerResource('map', parsedUrl.query),
      appCode: fetch(appCodeURL),
  ...
  _maybeStartPackageServer() {
    if (!this.packagerCacheDir) {
      return this._ensurePackageServer();
    }

    // If the directory doesn't exist, create it. And also start the React
    // Native packager because we know every request will be a cache miss.
    if (!fs.existsSync(this.packagerCacheDir)) {
      console.log(this.packagerCacheDir + ' does not exist, creating it...');
      return mkdirp.mkdirpAsync(this.packagerCacheDir)
        .then(() => this._ensurePackageServer());
    }

    console.log(this.packagerCacheDir + ' exists, not starting React Native packager packager for now.');
   ...
   * be actually ready.
   */
  _ensurePackageServer() {
    if (this._packagerPromise) {
      return this._packagerPromise;
    }

    this._packagerPromise = new Promise((resolve, reject) => {
